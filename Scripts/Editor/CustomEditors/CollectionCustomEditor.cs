...
namespace BrunoMikoski.ScriptableObjectCollections
{
    ...
    public class CollectionCustomEditor : Editor
    {
        ...
        {
            ScriptableObject collectionItem = SOCEditorUtility.AddNewItem(collection, itemType);
            ...
            if (ScriptableObjectCollectionUtility.IsTryingToGoToItem(out int targetIndex))
            {
                collectionItemListView.schedule.Execute(() =>
                {
                    ...
                    if (targetIndex < filteredItems.Count)
                    {
                        ...
                        SetExpandedStateForIndex(targetIndex, true);
                        collectionItemListView.ScrollToItem(targetIndex);
                    }
                }).ExecuteLater(100);
            }

            collectionItemListView.reorderable = false; // No reorder - folder-based ordering
        ...
        private void OnClickRemoveSelectedItems(MouseUpEvent evt)
        {
            ...
            else
            {
                ...
                foreach (ScriptableObject item in itemsToBeDeleted)
                    RemoveItemAtIndex(filteredItems.IndexOf(item), result == 2);
        ...
        private ScriptableObject AddNewItemOfType(Type targetType, string assetName = "", bool autoFocusForRename = true)
        {
            ...
            ScriptableObject newItem = SOCEditorUtility.AddNewItem(collection, targetType, assetName);
        ...
        private void ShowOptionsForIndex(MouseUpEvent evt, int targetIndex)
        {
            ...
            {
                menu.AddDisabledItem(new GUIContent("Copy Values"), false);
            }
            ...
            if (possibleCollections.Count > 0)
            {
                foreach (ScriptableObjectCollection otherCollection in possibleCollections)
                {
                    ...
                    menu.AddItem(
                        new GUIContent($"Move to {AssetDatabase.GetAssetPath(otherCollection).Replace("/", "\\").Replace("Assets", "").Replace(".asset", "")}"),
                        false,
                        () =>
                        {
                            if (selectedItemsCount > 0)
